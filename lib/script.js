...
/* DEFINITIONS */
const opcodes = {
    // Dummy value for native input (Numbers, Strings, Buffers, etc)
    ...
    ZFI_1: {
        scripts: {
            validation: {
                ...
                */
                structure: opcodes.NATIVE_INPUT + " ISNAMEUSED CONTINUETRUE GETITEMEPOCH CHAINEPOCH GREATERTHAN"
            }
        }
    }
}
...
function conformsToStandard(script, standard) {
    ...
    for (let i=0; i<nStandardScript.length; i++) {
        // Ignore native input (non-OPcodes)
        if (nStandardScript[i] === opcodes.NATIVE_INPUT) continue;
...
async function pushToStack(data, script, contextualData) {
    ...
    // ISNAMEUSED - Implemented from the ZFI-1 spec
        ...
        for (i=0; i<len; i++) {
            ...
            if (nItem.tx === contextualData.this.tx) continue;
            if (!nItem.contracts.validation) continue;
            if (!conformsToStandard(nItem.contracts.validation, ZFI_STANDARDS.ZFI_1)) continue;
            // Ensure the item is a ZFI-1 item; Other items and contracts can use the same name without conflicts.
            const itemNameBuffer = Buffer.from(nItem.name, "utf8");
