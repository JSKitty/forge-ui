    ...
    await asyncForEach(items, async (lItem) => {
        try {
            ...
            try {
                ...
                // Unexpected wallet error! We'll silently catch and ignore this for now
            }
        };
    });
...
let isUsingCoreConfig = false;
function startForge() {
    if (isInitializing) return;
    isInitializing = true;
    database.fromDisk("config.json", true).then(config => {
        /* Forge Config file (full configuration options) */
        if (!config || isUsingCoreConfig) {
            // Check if we can use the core config settings as a last resort
            if (Number(database.getConfigValue("server")) === 1) {
                // We'll fallback to using the core config, and extract values directly from it
                let forgeConf;
                try {
                    forgeConf = JSON.parse(JSON.stringify(config));
                } catch (e) { /* silent error, you saw nothing... */ }
                config = {
                    wallet: {
                        user: database.getConfigValue("rpcuser"),
                        pass: database.getConfigValue("rpcpassword"),
                        port: database.getConfigValue("rpcport", 26211)
                    }
                }
                if (forgeConf) {
                    // Individually recover information from an existing forge config
                    if (forgeConf.wallet) {
                        if (forgeConf.wallet.datadir)
                            config.wallet.datadir = forgeConf.wallet.datadir;
                        if (forgeConf.wallet.port && forgeConf.wallet.port > 0)
                            config.wallet.port = forgeConf.wallet.port;
                        if (forgeConf.wallet.address)
                            config.wallet.address = forgeConf.wallet.address;
                    }
                    if (forgeConf.forgeport && forgeConf.forgeport > 0)
                        config.forgeport = forgeConf.forgeport;
                    if (forgeConf.fullnode !== undefined)
                        config.fullnode = forgeConf.fullnode;
                    if (forgeConf.debug)
                        config.debug = forgeConf.debug;
                }
                console.log("Successfully derived RPC settings from the core config!");
            }
            // No Forge config and no core config, bail out!
            else {
                console.warn("- config.json is missing and couldn't find a core config to fallback to, if you're not using the Forge GUI wallet, you'll have to fix this manually.");
                isInitializing = false;
                return;
            }
        }
        ...
        /* Forge Port (int) */
        if (config.forgeport || config.forgePort) {
            // Default is 'forgeport', however an old software typo meant older configs may contain "forgePort",
            // ... make sure to recognize this option for a few months, to save on future user sanity.
            forgePort = config.forgeport || config.forgePort;
        } else {
        ...
        zenzo.call("ping").then(msg => {
            ...
            if (!isUsingCoreConfig) {
                console.error("Failed to connect to ZENZO-RPC, retrying with core config...");
                isUsingCoreConfig = true;
                isInitializing = false;
                startForge();
            } else {
                console.error("Failed to connect to ZENZO-RPC with both Forge and Core configs, you're on your own, soldier.");
                // Give up, with crushed pride
                rpcError();
            }
        });
    });
...
database.reloadConfigZC().then(success => {
    if (success) console.log("Successfully loaded core config file!");
    else console.warn("Failed to load core config file, falling back to config.json and defaults!");
    startForge();
})
...
async function setupForge(address, forgeOnly = false) {
    ...
    // If ZENZO Core contains already-existing RPC settings, re-use them as a priority
    if (Number(database.getConfigValue("server")) === 1) { 
        nConfig.wallet.user = database.getConfigValue("rpcuser");
        nConfig.wallet.pass = database.getConfigValue("rpcpassword");
        nConfig.wallet.port = database.getConfigValue("rpcport", 26211);
    }
