    ...
    connect(shouldPing) {
        ...
        if (getPeer(this.host) === null) {
            if (!shouldPing) {
                ...
                return superagent
                    .post(this.host + "/ping")
                    .send(params.protocolVersion) // Send peer our protocol
                    .then((res) => {
                        // Peer responded, check protocol validity
                        if (!params.isValidProtocol(res.text)) return console.warn("Peer " + this.host + " has an invalid protocol, cancelling connection...");
                        // Check the peer's protocol is within consensus of ours
                        let pver = params.parse(res.text);
                        if (!params.hasConsensus(pver)) return console.warn("Peer " + this.host + " doesn't meet local protocol consensus, cancelling connection...")
...
app.post('/ping', (req, res) => {
    ...
    if (ip !== "127.0.0.1") {
        ...
        if (req.peer !== null) {
            ...
            if (params.isValidProtocol(req.body)) {
                req.peer = new Peer(ip, req.body);
                req.peer.lastPing = Date.now();
                req.peer.connect(false);
            }
        }
        console.info('Received ping from "' + ip + '" (' + req.peer.index + ', protocol ' + req.body + ')');
    }
...
app.post('/forge/receive', (req, res) => {
    ...
    if (req.peer === null) {
        ...
        return console.warn("Peer " + ip + " tried to send us items without a handshake connection, ignoring...");
    }
