...
class CItem {
    ...
    constructor (strTx, strAddress, strName, nValue, strImage = "default", nTimestamp = -1, strSig = "", objPrev = null, objMetadata = {}, objContracts = {}, nVersion = 0) {
        // Sanity checks
    ...
    getValidityLevel() {
        ...
        else if (!this.lastValidation.successful)
...
function approveItem(item) {
    ...
    item.lastValidation.successful = true;
    item.lastValidation.consequtiveFailures = 0;

    // First, check the pending list for the item
    let itemsToValidate = getPendingItems();
    for (let i=0; i<itemsToValidate.length; i++) {
        if (item.strTx === itemsToValidate[i].strTx) {
            wasFound = true;
            itemsToValidate[i].lastValidation.successful = true;
            itemsToValidate[i].lastValidation.consequtiveFailures = 0;
        ...
        if (item.strTx === unsignedItems[i].strTx) {
            if (item.strSig && !unsignedItems[i].strSig) {
                if (item.strSig.length > 0) {
                    ...
                    unsignedItems[i].lastValidation.successful = true;
                    unsignedItems[i].lastValidation.consequtiveFailures = 0;
...
function disproveItem(item) {
    ...
    for (i=0; i<len; i++) {
        if (item.strTx === itemsToValidate[i].strTx) {
            // Check if this is a pending item
            if (item.getValidityLevel() === ValidityLevel.PENDING) {
                // Increment the consequtive failures counter
                if (!item.lastValidation.successful) item.lastValidation.consequtiveFailures++;
                // If the item has failed validation three times, it gets erased
                if (item.lastValidation.consequtiveFailures >= 3) {
                    eraseItem(item, true);
            ...
            item.lastValidation.successful = false;
        }
    }
}
...
app.post('/forge/sync', (req, res) => {
    ...
    if (Number(req.body.len) != items.length) {
        req.peer = net.getPeer("http://" + ip);
        net.receivedPing();
    ...
    let obj = {items: util.cleanItems(items, true), pendingItems: [], unsignedItems: [], smeltedItems: itemsSmelted};
...
app.post('/forge/items', (req, res) => {
    ...
    let obj = {items: items, pendingItems: [], unsignedItems: []};
    res.json(obj);
});
