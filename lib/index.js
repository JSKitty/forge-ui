...
let appdataZC = process.env.APPDATA + '/Zenzo/' || (process.platform == 'darwin' ? process.env.HOME + 'Library/Application Support/Zenzo/' : process.env.HOME + '/.zenzo/');
        ...
        if (!wasItemSmelted(smelt.tx)) {
            ...
            zenzo.call("verifymessage", smelt.address, smelt.sig, "smelt_" + smelt.tx).then(isGenuine => {
                if (isGenuine) {
                    ...
                    smeltItem(smelt.tx, smelt.sig).then(smelted => {
    ...
    /* A peer wants to smelt an item */
        ...
        zenzo.call("verifymessage", smeltedItem.address, messageQueue[0].content.sig, "smelt_" + smeltedItem.tx).then(isGenuine => {
            if (isGenuine) {
                ...
                // Begin the local smelt process for the item
                smeltItem(smeltedItem.tx, messageQueue[0].content.sig).then(smelted => {
...
async function smeltItem (item, signature = null) {
    ...
    itemsSmelted.push({tx: item, address: thisItem.address, sig: signature});
