...
// A quick check to know if a raw item us using the pre-ZFI-revamp format
...
app.post('/forge/receive', (req, res) => {
    ...
    validateItemBatch(res, util.cleanItems(nItems.items.concat(nItems.pendingItems, nItems.unsignedItems)), true).then(done => {
...
app.post('/forge/sync/hashes', (req, res) => {
    ...
    getItemHashes(true, true).then(ourHashes => {
        ...
        itemsToSend = util.cleanItems(itemsToSend);
...
function exchangeItems(peer) {
    ...
    getItemHashes(true, true).then(itemHashes => {
        ...
        .then((res) => {
            ...
            validateItemBatch(null, util.cleanItems(data.itemsSent), false).then(done => {
                if (done) {
            ...
            if (data.hashesWanted.length > 0) {
                ...
                data.hashesWanted.forEach(nHash => {
                    ...
                    if (foundHash) {
                        itemsToSend.push(getItem(foundHash.tx, true, true));
                        console.error("Peer " + peer.host + " is missing our item, sending...: " + foundHash.tx);
                    }
                });
            ...
            for (let i = 0; i < nDiskItems.length; i++) {
                ...
                try {
                    ...
                    if (isOldItemFormat(objZFI)) {
                        // This item is old, lets attempt to upgrade it...
            ...
            if (unsignedItem.address === addy) {
                ...
                zenzo.call("signmessage", addy, unsignedItem.tx).then(sig => {
                    if (sig && sig.length > 5) {
                        ...
                        if (_.isNil(unsignedItem.timestamp)) unsignedItem.timestamp = Math.floor(Date.now() / 1000);
