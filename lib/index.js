...
function addToValidationQueue(nItem, revalidate = false) {
    ...
    if (fullNode && !revalidate && !isInvalid && !isSmelted && !isUnsigned && !_.isNil(cItemTest)) {
        ...
        if ((cItemTest.nVersion + 1) > nItem.nVersion)
            isIgnored = true;
    }
...
function approveItem(nItem) {
    ...
    if (!hasLocally || (hasLocally && (prevItem.nVersion + 1) > nItem.nVersion)) {
...
app.post('/forge/profiles', (req, res) => {
    let objProfiles = [], allProfiles = getAllProfiles(true);
    for (const profile of allProfiles) {
        objProfiles.push(profile.formatAsObject());
    }
    res.send(JSON.stringify(objProfiles));
});
...
app.post('/forge/profile', (req, res) => {
    if (req.body.name && req.body.name.length >= 1) {
        ...
        res.send(JSON.stringify(objProfile.formatAsObject()));
    }
});
