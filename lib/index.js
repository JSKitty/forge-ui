...
class CItem {
    ...
    constructor (strTx, strAddress, strName, nValue, strImage = "default", nTimestamp = -1, strSig = "", objPrev = null, objMetadata = {}, objContracts = {}) {
        // Sanity checks
    ...
    getValidityLevel() {
        ...
        else if (this.lastValidation.successful === false)
    ...
    needsValidating() {
        // If the 'lastValidation' is 'this.priority' seconds old, we must signal a re-validation
        if (util.epoch() >= this.lastValidation + this.priority)
            return true;
        else
            return false;
    }
...
function parseItem(objZFI) {
    ...
    if (isOldItemFormat(objZFI)) {
        ...
        cZFI = new CItem(objZFI.tx, objZFI.address, objZFI.name, objZFI.value,
                         ...
                         objZFI.metadata, objZFI.contracts);
    } else {
        ...
        cZFI = new CItem(objZFI.strTx, objZFI.strAddress, objZFI.strName, objZFI.nValue,
                         ...
                         objZFI.objMetadata, objZFI.objContracts);
    }
    return cZFI;
}
...
function addToValidationQueue(nItem, revalidate = false) {
    ...
    if (!isInvalid) {
        ...
        if (!_.isNil(nItem.strSig) && nItem.strSig.length > 1)
            isUnsigned = false;
    }
    ...
    if (fullNode && !revalidate && !isInvalid && !isSmelted && !isUnsigned && hasItem(nItem.strTx)) {
        isIgnored = true;
    }
...
function runValidationQueue() {
    ...
    if (lockedLen === validationQueue.list.length) {
        ...
        for (i=0; i<len; i++) {
            if (debug("validations")) console.info(" - lockedList["  + i + "] = " + validationQueue.lockedList[i].strTx + ", list[" + i + "] = " + validationQueue.list[i].strTx);
            if (validationQueue.lockedList[i].strTx === validationQueue.list[i].strTx) {
    ...
    asyncForEach(validationQueue.lockedList, async (item) => {
        ...
        let isUnsigned = (!item.strSig ? true : false);
...
function isItemContentValid (nItem) {
    ...
    if (nItem.strTx.length !== 64) {
        console.error(nItem);
        console.error("Forge: Received invalid item, TX length is not 64. (Has: " + nItem.strTx.length + ")");
        return false;
    }
    // Address length is fixed (34 bytes)
    if (nItem.strAddress.length !== 34) {
        console.error("Forge: Received invalid address, length is not 34. (Has: " + nItem.strAddress.length + ")");
        return false;
    }

    // Name is custom, human-readable but must be between 1 and 50 chars
    if (nItem.strName.length < 1) {
        console.error("Forge: Received invalid name, length is below 1. (Has: " + nItem.strName.length + ")");
        return false;
    } else if (nItem.strName.length > 50) {
        console.error("Forge: Received invalid name, length is over 50. (Has: " + nItem.strName.length + ")");
        return false;
    }

    // Value is custom but must be atleast 0.001 ZNZ
    if (nItem.nValue < 0.001) {
        console.error("Forge: Received invalid item, value is below minimum. (Has: " + nItem.nValue + ", Min: 0.001)");
        return false;
    }

    // Ensure the item has no unexpected / illegal keys
    const acceptedKeys = [
        "strTx", "nTimestamp", "objPrev", "strSig", "strAddress", "strName", "strImage", "nValue", "objMetadata", "objContracts", // Public (clean) keys
        "vout", "isUnsigned", "invalidScore", "lastValidation", "priority", // Private (unclean) keys, these should NOT come from peers, this allows for more efficient internal validation
    ...
    if (nItem.objContracts && !_.isEmpty(nItem.objContracts)) {
        let nRaw;
        try {
...
async function isItemValid (nItem, isUnsigned, approve = false) {
    try {
        ...
        if (wasItemSmelted(nItem.strTx)) {
            ...
            if (debug("validations")) console.error("Forge: Item '" + nItem.strName + "' was previously smelted.");
            return false;
        }
        ...
        if (!_.isEmpty(nItem.objContracts) && !_.isNil(nItem.objContracts.validation)) {
            ...
            if (!wasItemSmelted(smelt.strTx)) {
                ...
                zenzo.call("verifymessage", smelt.strAddress, smelt.strSig, "smelt_" + smelt.strTx).then(isGenuine => {
                    if (isGenuine) {
                        ...
                        smeltItem({tx: smelt.strTx, address: smelt.strAddress}, smelt.strSig).then(smelted => {
...
function upgradeItem(nItem, rawTx) {
    ...
    if (!nItem.nTimestamp || nItem.nTimestamp && nItem.nTimestamp < 1) {
        nItem.nTimestamp = rawTx.blocktime;
        hasUpdated = true;
    }
...
function approveItem(item) {
    ...
    for (let i=0; i<itemsToValidate.length; i++) {
        if (item.strTx === itemsToValidate[i].strTx) {
            wasFound = true;
            items.push(item);
            itemsToValidate.splice(i, 1);
            console.info("A pending item has been approved!\n - Item '" + item.strName + "' (" + item.strTx + ") has been approved and appended as a verified item.");
        }
    }
    ...
    for (let i=0; i<unsignedItems.length; i++) {
        if (item.strTx === unsignedItems[i].strTx) {
            if (item.strSig && !unsignedItems[i].strSig) {
                if (item.strSig.length > 0) {
                    item.signedByReceiver = true;
                    unsignedItems.splice(i, 1);
                    console.info("An unsigned item has been signed by it's owner!\n - Item '" + item.strName + "' (" + item.strTx + ") removed from unsigned list");
                }
            }
    ...
    if (!_.isNil(itemFromUnsigned)) {
        eraseItem(itemFromUnsigned.objPrev[0], true);
    }
    ...
    if (!wasFound) {
        ...
        if (!wasFound && !_.isNil(item.strSig)) {
            // Case 1: New valid signed item
            delete item.signedByReceiver;
            items.push(item);
            console.info("An item has been added and approved!\n - Item '" + item.strName + "' (" + item.strTx + ") has been approved and appended as a verified item.");
        } else if (!wasFound && !item.signedByReceiver && !item.strSig && _.isNil(itemFromUnsigned)) {
            // Case 2: New valid unsigned item
            delete item.signedByReceiver;
            console.info("An unsigned item has been added to the unsigned list!\n - Item '" + item.strName + "' (" + item.strTx + ") has been approved and appended as an unsigned item.");
            unsignedItems.push(item);
        } else if (!_.isNil(itemFromUnsigned) && item.signedByReceiver && itemFromUnsigned.strTx === item.strTx) {
            ...
            console.warn(item);
            console.warn("WARNING: approveItem() for (" + item.strName + ", " + item.strTx + ") met no conditions, this item has unexpected properties!\n --- Please report this to the developer! ---");
        }
    }
}
...
function disproveItem(item) {
    ...
    for (let i=0; i<itemsToValidate.length; i++) {
        if (item.strTx === itemsToValidate[i].strTx) {
            eraseItem(item, true);
            if (debug("validations")) console.warn("A bad item has been erased!\n - Item '" + item.strName + "' (" + item.strTx + ") has been erased from the local database.");
        }
    }
    ...
    for (let i=0; i<items.length; i++) {
        if (item.strTx === items[i].strTx) {
            ...
            items.splice(i, 1);
            if (debug("validations")) console.warn("An item has been disproved!\n - Item '" + item.strName + "' (" + item.strTx + ") has been removed as a verified item and is now pending.");
        }
    }
}
...
function eraseItem(item, includeUnsigned = false) {
    ...
    for (let i=0; i<validationQueue.list.length; i++) {
        if (item.strTx === validationQueue.list[i].strTx) {
            validationQueue.list.splice(i, 1);
        }
    }
...
function wasItemSmelted(item) {
    if (_.find(itemsSmelted, {'strTx': item}))
        return true;
...
function hasItem(item) {
    if (_.find(items, {'strTx': item})) return "items";
    if (_.find(itemsToValidate, {'strTx': item})) return "itemsToValidate";
    return false;
}
...
function addInvalidationScore(item, score) {
    ...
    for (let i=0; i<itemsToValidate.length; i++) {
        if (item.strTx === itemsToValidate[i].strTx) {
            ...
            if (debug("validations")) console.info("An invalidation score of '" + score + "' has been applied to '" + item.strName + "', now totalling '" + itemsToValidate[i].invalidScore + "' invalidation score.");
            if (item.invalidScore >= maxInvalidScore) {
                disproveItem(item);
...
function getItem(itemArg, includePending = false, includeUnsigned = false) {
    for (let i=0; i<items.length; i++) {
        if (items[i].strTx === itemArg) return items[i];
    }
    ...
    if (includePending) {
        for (let i=0; i<itemsToValidate.length; i++) {
            if (itemsToValidate[i].strTx === itemArg) return itemsToValidate[i];
        }
    }
    ...
    if (includeUnsigned) {
        for (let i=0; i<unsignedItems.length; i++) {
            if (unsignedItems[i].strTx === itemArg) return unsignedItems[i];
        }
    }
...
function getItemsByName(itemArg, includePending = false, includeUnsigned = false) {
    ...
    for (let i=0; i<items.length; i++) {
        if (items[i].strName.includes(itemArg)) retItems.items.push(items[i]);
    }
    ...
    if (includePending) {
        for (let i=0; i<itemsToValidate.length; i++) {
            if (itemsToValidate[i].strName.includes(itemArg)) retItems.pendingItems.push(itemsToValidate[i]);
        }
    }
    ...
    if (includeUnsigned) {
        for (let i=0; i<unsignedItems.length; i++) {
            if (unsignedItems[i].strName.includes(itemArg)) retItems.unsignedItems.push(unsignedItems[i]);
        }
    }
...
function updateItem (itemArg) {
    ...
    for (let i=0; i<itemsToValidate.length; i++) {
        if (itemsToValidate[i].strTx === itemArg.strTx) {
...
app.post('/forge/sync', (req, res) => {
    ...
    let obj = {items: util.cleanItems(items, true), pendingItems: itemsToValidate, unsignedItems: unsignedItems, smeltedItems: itemsSmelted};
...
app.post('/forge/inventory', (req, res) => {
    ...
    for (let i=0; i<items.length; i++) {
        if (items[i].strAddress === addy) ourItems.push(items[i]);
    }

    // Find our pending items (Mempool items, these can be optionally included in fast-paced games)
    for (let i=0; i<itemsToValidate.length; i++) {
        if (itemsToValidate[i].strAddress === addy) ourPendingItems.push(itemsToValidate[i]);
    }
...
app.post('/forge/account', (req, res) => {
    if (net.getPeers().length === 0 || safeMode) return res.json({error: "Account information is unavailable while the Forge is in Safe Mode and/or Offline."});
    let ip = net.cleanIP(req.ip);
...
app.post('/forge/create', (req, res) => {
    ...
    zenzo.call("sendtoaddress", addy, Number(req.body.amount.toFixed(8))).then(txid => {
        ...
        zenzo.call("signmessage", addy, txid).then(sig => {
            ...
            if (_.isNil(nItem.objMetadata) || _.isEmpty(nItem.objMetadata)) {
                // To save a little bit of space, metadata can be completely deleted if empty
                delete nItem.objMetadata;
            }
                    ...
                    zenzo.call("sendrawtransaction", signedTx.hex).then(txid => {
                        //zenzo.call("signmessage", addy, txid).then(sig => {
                            let nItem = {
                                ...
                                address: req.body.to,
                                name: tItem.strName,
                                image: tItem.strImage,
                                value: formatNum(tItem.nValue - 0.001),
                                contracts: tItem.objContracts
                            }
                            if (tItem.objMetadata && !_.isNil(tItem.objMetadata)) {
                                // Add metadata to the transferred item if the old one had any
                                nItem.objMetadata = tItem.objMetadata;
                            }
    ...
    zenzo.call("gettransaction", smeltingItem.strTx).then(rawtx => {
        zenzo.call("lockunspent", true, [{"txid": smeltingItem.strTx, "vout": rawtx.details[0].vout}]).then(didUnlock => {
            ...
            zenzo.call("signmessage", addy, "smelt_" + smeltingItem.strTx).then(sig => {
                smeltItem({tx: smeltingItem.strTx, address: addy}, sig);
                ...
                for (let i=0; i<lockedItems.length; i++) {
                    if (lockedItems[i].strTx === smeltingItem.strTx) {
...
app.post('/forge/items', (req, res) => {
    ...
    itemsToValidate.sort(function(a, b){return b.nTimestamp-a.nTimestamp});
    unsignedItems.sort(function(a, b){return b.nTimestamp-a.nTimestamp});
    let obj = {items: items, pendingItems: itemsToValidate, unsignedItems: unsignedItems};
    res.json(obj);
});

// Start a hash-comparison item exchange with a peer
function exchangeItems(peer) {
    ...
    getItemHashes(true, true).then(itemHashes => {
        return superagent
        .post(peer.host + "/forge/sync/hashes")
        .send(itemHashes)
        .then((res) => {
            ...
            if (debug("net")) console.info(`Peer "${peer.host}" (${peer.index}) sent (${data.itemsSent.length} Items)`);
            // (tmp, needs optimizing) smelts are included in the hashes for now
            validateSmelts(data.smeltedItems);
            validateItemBatch(null, parseItems(data.itemsSent), false).then(done => {
                if (done) {
                    // Only display net + validations stats IF anything actually changed
                    if ((done.accepted + done.ignored + done.rejected) > 0) {
                        if (debug("net")) console.info(`Synced with peer "${peer.host}", we now have ${items.length} valid, ${itemsToValidate.length} pending items & ${unsignedItems.length} unsigned items!`);
            ...
            if (data.hashesWanted.length > 0) {
                ...
                data.hashesWanted.forEach(nHash => {
                    ...
                    if (foundHash) {
                        itemsToSend.push(getItem(foundHash.strTx, true, true));
                        console.error("Peer " + peer.host + " is missing our item, sending...: " + foundHash.strTx);
                    }
                });
                if (itemsToSend.length > 0) {
                    peer.sendItems(itemsToSend).then(done => {
                        if (debug("net")) console.info("Sent peer \"" + peer.host + "\" " + itemsToSend.length + " requested items!");
                    });
                }
            }
        })
    ...
    if (messageQueue[0].content.header === "smelt") {
        ...
        zenzo.call("verifymessage", smeltedItem.strAddress, messageQueue[0].content.sig, "smelt_" + smeltedItem.strTx).then(isGenuine => {
            if (isGenuine) {
                ...
                // Begin the local smelt process for the item
                smeltItem({tx: smeltedItem.strTx, address: smeltItem.strAddress}, messageQueue[0].content.sig).then(smelted => {
...
async function smeltItem (item, signature = null) {
    ...
    if (!_.isNil(thisItem) && addy === thisItem.strAddress) {
        try {
            ...
            console.error("- Unable to unlock smelted item collateral (" + thisItem.strName + ", " + thisItem.strTx + ")");
        }
    }
...
async function lockCollateralUTXOs() {
    ...
    if (lockedItems.length > 0) {
        let nConfigZC = "";
        for (let i=0; i<lockedItems.length; i++) {
            nConfigZC += "\r\n" + lockedItems[i].strName.replace(/ /g, "_") + " " + lockedItems[i].strTx + " " + lockedItems[i].vout;
        }
        ...
        unsignedItems.forEach(unsignedItem => {
            if (unsignedItem.strAddress === addy) {
                ...
                zenzo.call("signmessage", addy, unsignedItem.strTx).then(sig => {
                    if (sig && sig.length > 5) {
                        unsignedItem.strSig = sig;
                        if (_.isNil(unsignedItem.nTimestamp)) unsignedItem.nTimestamp = Math.floor(Date.now() / 1000);
