...
async function validateItemBatch (res, nItems, reply) {
    console.info("Starting item batch validation...");
    console.time("Batch validation for " + nItems.length.toString() + " items");
    let validationStats = {
    ...
    await asyncForEach(nItems, async (nItem) => {
        ...
        if (nItem.tx.length !== 64) {
            console.warn("Forge: Received invalid item, TX length is not 64. (Has: " + nItem.tx.length + ")");
            isInvalid = true;
        }
        if (nItem.sig) isUnsigned = false;
        if (nItem.address.length !== 34) {
            console.warn("Forge: Received invalid address, length is not 34. (Has: " + nItem.address.length + ")");
            isInvalid = true;
        }
        if (nItem.name.length < 1) {
            console.warn("Forge: Received invalid name, length is below 1. (Has: " + nItem.name.length + ")");
            isInvalid = true;
        }
        if (nItem.value < 0.001) {
            console.warn("Forge: Received invalid item, value is below minimum. (Has: " + nItem.value + ", Min: 0.001)");
            isInvalid = true;
        }
        if (nItem.hash.length !== 64) {
            console.warn("Forge: Received invalid item, hash length is not 64. (Has: " + nItem.hash.length + ")");
            isInvalid = true;
        }
        ...
        if (!isInvalid && wasItemSmelted(nItem.hash)) {
            if (debug("validations")) console.error("Item (" + nItem.name + ") has been previously smelted, ignoring");
            isSmelted = true;
        }

        // Check if we already own the item
        if (!isInvalid && !isSmelted && hasItem(nItem.tx)) {
            if (debug("validations")) console.error("Already own (" + nItem.name + "), skipping double-validation and ignoring");
            isIgnored = true;
        }

        // Check if the item's contents are genuine
        if (!isInvalid && !isIgnored && !isSmelted && !isUnsigned || // Signed items
            !isInvalid && isUnsigned) { // Unsigned items
            let valid = await isItemValid(nItem, isUnsigned, true);
            if (!valid) {
                if (debug("validations")) console.error("Forge: Received item is not genuine, rejected.");
                isInvalid = true;
            }
        }

        if (isIgnored || isSmelted)
    ...
    console.timeEnd("Batch validation for " + nItems.length.toString() + " items");
