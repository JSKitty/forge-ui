...
function addToValidationQueue(nItem, revalidate = false) {
    ...
    if (!isInvalid && wasItemSmelted(nItem.hash)) {
        if (debug("validations")) console.warn("Item (" + nItem.name + ") has been previously smelted, ignoring");
        isSmelted = true;
    }

    // Check if we already own the item
    if (!revalidate && !isInvalid && !isSmelted && hasItem(nItem.tx)) {
        if (debug("validations")) console.warn("Already own (" + nItem.name + "), skipping double-validation and ignoring");
        isIgnored = true;
    }
...
function runValidationQueue() {
    ...
    if (lockedLen === validationQueue.list.length) {
        ...
        for (i=0; i<len; i++) {
            console.info(" - lockedList["  + i + "] = " + validationQueue.lockedList[i].tx + ", list[" + i + "] = " + validationQueue.list[i].tx);
    ...
    asyncForEach(validationQueue.lockedList, async (item) => {
        ...
        let valid = await isItemValid(item, item.isUnsigned, true);
        if (!valid) {
            if (debug("validations")) console.error("Forge: Received item is not genuine, rejected.");
        }
        if (i+1 === lockedLen) {
            // No more items in the queue
            console.timeEnd("Batch validation for " + lockedLen + " items");
...
async function lockCollateralUTXOs() {
    ...
    await asyncForEach(items, async (lItem) => {
        try {
            if (!lockedItems.includes(lItem.tx) && lItem.address === addy) {
                ...
                if (didLock) {
                    console.info("- Item (" + lItem.name + ") collateral was successfully locked in ZENZO Coin Control.");
                    lockedItems.push(lItem.tx);
                }
            }
        } catch (e) {console.warn("- Item (" + lItem.name + ") failed to lock (" + e + ")")};
    });
    return true;
}
...
let janitor = setInterval(function() {
            ...
            if (unsignedItem.address === addy) {
                ...
                zenzo.call("signmessage", addy, unsignedItem.tx).then(sig => {
                    if (sig) {
                        ...
                        if (debug("validations")) console.info(" - Item signed successfully!");
                    } else {
    ...
    lockCollateralUTXOs().then(locked => {
        console.info("Now " + lockedItems.length + " UTXOs locked!");
    });
}, 5000);
