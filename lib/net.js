...
const seednodes = ["144.91.87.251:8000", "164.68.102.142:45001", "164.68.115.32:45001", "45.77.128.182:9002", "89.40.5.16:9002"];
...
class Peer {
    constructor(host, protocol) {
        ...
        this.syncedItems = []; // A list of item pointers, these are items that are confirmed to be shared with this peer
    }
    ...
    sendItems(itemsToSend = null, smelts = null) {
        ...
        itemsToSend = {
            items: util.cleanItems(itemsToSend, true),
            pendingItems: [],
            unsignedItems: [],
    ...
    hasItem(nItem) {
        // Checks if we have the same (or a higher) version of this item
        for (const nHeader of this.syncedItems) {
            if (nHeader.strTx === nItem.strTx && nHeader.nVersion >= nItem.nVersion) return true;
        }
        // No match found; or our version is older than the compared one
        return false;
    }

    eraseItem(nItem) {
        let i = 0, len = this.syncedItems.length;
        for (i=0; i<len; i++) {
            if (this.syncedItems[i] && this.syncedItems[i].strTx === (nItem.strTx || nItem.tx)) {
                this.syncedItems.splice(i, 1);
            }
        }
    }
}
...
// Removes an item from each peer's context
function eraseItemFromPeers (nItem) {
    peers.forEach(peer => {
        peer.eraseItem(nItem);
    });
}
...
exports.eraseItemFromPeers = eraseItemFromPeers;
